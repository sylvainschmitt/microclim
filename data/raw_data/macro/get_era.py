...
# Weather station
ee.Initialize(project="ee-sylvainmschmitt", opt_url='https://earthengine-highvolume.googleapis.com')
ic = ee.ImageCollection("ECMWF/ERA5_LAND/HOURLY").filter(ee.Filter.date('1995-04-01', '2025-01-01'))
pt = ee.Geometry.Point(-52.6, 5.17)
ds = xr.open_mfdataset([ic], 
                       engine='ee', 
                       projection=ic.first().select(0).projection(), 
                       geometry=pt,
...
ds = ds[['temperature_2m']]
ds['tas'] = ds['temperature_2m'] - 273.15
ds_day = ds.groupby("time.date").mean()
ds_df = ds_day[['tas']].to_dataframe()
