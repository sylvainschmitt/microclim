...
```{r}
#| eval: false
sensors <- unique(vroom("outputs/tomst_data.tsv")$tomst_sensornum)
raw <- lapply(sensors, function(s) {
  vroom("outputs/tomst_data.tsv") %>% 
  filter(tomst_sensornum == s) %>% 
  gather(variable, value, -site, -date, -tomst_sensornum) %>% 
  group_by(site, variable, tomst_sensornum, date = as_date(date)) %>% 
  summarise(value = median(value))
}) %>% bind_rows()
write_tsv(raw, "outputs/tomst_data_day_raw.tsv")
```

```{r}
#| message: false
#| warning: false
raw <- read_tsv("outputs/tomst_data_day_raw.tsv")
cleaned <- raw %>% 
...
#| warning: false
raw %>% 
  filter(!(variable %in% c("swc", "moisture"))) %>% 
