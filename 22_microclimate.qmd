...
The power similarly increases during the dry season. Interestingly we can see that the approach is pretty resilient to unfiltered data even if we can easily detect anomalies in soil power due to captors removed from the soil.
...
Comparing HOBO power against ERA5-Land power for the period 24hr we see mainly a buffer effect with decreased amplitude except for high air temperature, could this be due to a lack of correction of ERA5-Land data?
...
However, the buffer ratio did not correlated to total plant area index. Its computation should be checked and compared to the field categories. Maybe the buffer is wrong.
...
The buffer ratio correlated well to a proxy of canopy height but in the opposite direction. The vegetation metrics should definitely be checked.

```{r ratio_h}
#| message: false
#| warning: false
#| fig-cap: "Amplitude ratio for the period 24-hr along canopy hieght proxy."
pad <- read_tsv("data/derived_data/sensors_pad_xy.tsv") %>% 
  rename(x = x_p, y = y_p) %>%
  left_join(read_tsv("data/derived_data/pad.tsv")) %>% 
  group_by(id) %>% 
...
ratios %>% 
  left_join(pad) %>% 
  ggplot(aes(z, ratio, col = season)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method = "lm") +
  ggpubr::stat_cor() +
  theme(legend.position = "bottom") +
  xlab("Height of the max PAD value [m]") +
  ylab("Mean Amplitude ratio for period 24") +
  facet_wrap(~ type)
```
