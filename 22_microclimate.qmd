...
guyaflux <- readxl::read_excel("data/raw_data/guyaflux/TourAFlux-2023-2024.xlsx", 
  ...
  rename_all(tolower) %>% 
  rename(hr_55 = `hr(55)`, snet = rg, pr = pluie) %>% 
  select(date, hr_55, snet, pr) %>% 
...
temp %>% 
  ...
  mutate(across(c("ratio", "hr_55", "snet", "pr"), scale)) %>% 
  lm(ratio ~ season + hr_55 + snet + pr, data = .) %>% 
  sjPlot::tab_model()
```
