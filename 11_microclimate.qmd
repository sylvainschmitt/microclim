...
> ⚠️ I did not use the most recent data retrieval (30/09/2025) as many sensors were missing. I need to discuss with Léa Françoise what we have and how to best assemble the different time series.
...
read_tsv("data/derived_data/tomst.tsv") %>% 
  ...
  filter(date > as_datetime("2022-11-01 00:00:00")) %>% 
  filter(date < as_datetime("2023-04-01 00:00:00")) %>% 
  ggplot(aes(as.character(id), t_air)) +
  geom_boxplot() +
  theme_bw() +
  xlab("") + ylab("Temperature [ °C ]") +
  scale_color_discrete(guide = "none")
...
> The sensors do not seem to require calibration; however, it appears that we have two sets with different conditions. ⚠️ This needs to be checked.
...
#| message: false
#| warning: false
#| fig-cap: "Sensor 1 raw data. The yellow area indicates the dry season."
read_tsv("data/derived_data/tomst.tsv") %>% 
  filter(date >= as_datetime("2023-04-01 00:00:00")) %>% 
  filter(id == 1) %>% 
...
#| message: false
#| warning: false
read_tsv("data/raw_data/micro/test_sensor-project.tsv") %>% 
  select(-comment, -defect, -field) %>% 
  filter(date >= as_datetime("2023-04-01 00:00:00")) %>% 
  filter(id == 1) %>% 
  gather(variable, value, -sensornum, -date, -id) %>% 
  ggplot(aes(date, value)) +
